...
document.getElementById("title").addEventListener("click", () => {
    ...
    if(isSpeaking == false && isTyping == false)
    {
        typeWriter();
...
document.getElementById("mode-dropdown").addEventListener("click", () => {
    ...
    type_iterator = 0;
    perform_clicked = 0;
    txt = 'In plainsong, there are eight modes. Modes 1, 3, 5, and 7 are the "authentic" modes, and modes 2, 4, 6, and 8 are the "plagal" modes. Each authentic mode has a relative plagal. Relative modes share the same "Do" and final. The placement of the dominant varies among modes.';
    if(isSpeaking == false && isTyping == false)
    {
        typeWriter();
        if(monks_closed.style.zIndex == "-1")
...
function read_svg_element()
{
    ...
    for(let i=0; i<punctum.length; i++)
    {
        punctum[i].addEventListener("click", () => {
            ...
            {
                txt='Punctum Mora: A punctum with a dot. The dot doubles the value of the punctum.';//Goodchild,24//
            }
            type_iterator = 0;
            perform_clicked = 0;
            if(isSpeaking == false && isTyping == false)
...
function typeWriter()
{
    ...
    {
        document.getElementById("info-text").innerHTML += txt.charAt(type_iterator);
